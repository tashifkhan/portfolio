...
type ResponsibilityItem = {
	...
	organization: string;
	duration: string;
	type: "treasurer" | "executive" | "secretary" | "mentor" | string;
};
...
export default function EditResponsibilitiesForm({
	...
	const normalized = useMemo<DocShape | null>(() => {
		...
		if (Array.isArray(initialData) && initialData.length > 0) {
			...
			if (d && (d.educationData || d.responsibilitiesData))
		...
		const hydrate = async () => {
			try {
				...
				if (normalized) {
					...
					if (!res.ok) throw new Error("Failed to fetch responsibilities data");
				...
				if (!ignore) setError(e?.message ?? "Failed to load responsibilities");
			} finally {
	...
	const updateField = (
		index: number,
		field: keyof ResponsibilityItem,
		value: string
	) => {
		setResponsibilities((prev) => {
			const next = [...prev];
			next[index] = { ...next[index], [field]: value } as ResponsibilityItem;
			return next;
		});
	};
	...
	const addEntry = () => {
		setResponsibilities((prev) => [
			...prev,
			{ title: "", organization: "", duration: "", type: "executive" },
		]);
	};
	...
	const isValid = () =>
		responsibilities.every(
			(r) => r.title && r.organization && r.duration && r.type
		);
	...
	const handleSave = async () => {
		try {
			...
			if (!isValid()) {
				setError(
					"Please fill title, organization, type and duration for all entries"
				);
				setSaving(false);
				return;
			}
			...
			if (!res.ok) {
				...
				throw new Error(data?.error || "Failed to update responsibilities");
			}
			onSuccess();
		} catch (e: any) {
			setError(e?.message ?? "Failed to update responsibilities");
		} finally {
			setSaving(false);
		}
	};
	...
	return (
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				<h4 className="text-lg font-mono">Responsibilities</h4>
				<Button
					onClick={addEntry}
